...
def validate_dim_album_data(dwh_dim_table: pd.DataFrame) -> Dict[str, Any]:
    """
    Validate album data stored in a PostgreSQL DWH dimension table.

    Validation Rules:
    1. created_at should be after first_heard.

    Args:
        dwh_dim_table: pd.DataFrame - DataFrame containing the DWH dimension album data.

    Returns:
        dict: Validation results including success status, passed/failed checks, warnings, and summary.
    """

    logger.info("Validating PostgreSQL DWH dimension album data...")
